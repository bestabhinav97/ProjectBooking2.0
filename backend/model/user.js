...
module.exports.getPublicUserById = async (userId) => {
  try {
    ...
    // If the profileImageUrl column is missing in the database, fall back
    // to selecting the rest of the public fields so login doesn't fail.
    if (
      error &&
      error.code === "ER_BAD_FIELD_ERROR" &&
      String(error.sqlMessage || "").includes("profileImageUrl")
    ) {
      const [rows] = await db.execute(
        `SELECT userId, firstName, lastName, birthDate, zipCode, country,
